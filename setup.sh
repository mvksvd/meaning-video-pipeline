...
[ -n "$APIFY_API_TOKENS" ] && ok "APIFY_API_TOKENS загружены (скаут трендов)" || no "APIFY_API_TOKENS (опционально: скаут трендов, SETUP.md шаг 7б)"
echo "— Данные:"
[ -f sources.json ] && ok "sources.json (пул источников)" || no "sources.json (cp sources.example.json sources.json — для скаута)"
